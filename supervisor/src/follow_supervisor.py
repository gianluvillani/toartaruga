...
def follow_tn(path_available = False, danger = 0, danger_threshold=0.5, another_car= False):
	...
	return 'FOLLOW', False, 'FOLLOW'
        ...
        return 'STOP', True, 'FOLLOW'

def log_state(state, transited, previous_state):
	rospy.logerr("In state: %s, transited: %s, Previous state: %s", state, transited, previous_state)
...
class StateMachine:
	def __init__(self, init_state, states_tn):
		...
		self.command_controller_follow_top = rospy.get_param(rospy.get_name() + "/command_controller_follow_topic") #add in the launch file
...
#		self.sub_another_car=rospy.Subscriber("Bool_another_car", Bool, self.callback_another_car)
		...
		self.pub_command_controller_follow = rospy.Publisher(self.command_controller_follow_top, Bool) #write in the PID controller
...
#		self.another_car=bool_another_car.data
	...
	def advance_machine(self):
		...
		if self.transited:
			rospy.logerr('NEW STATE: %s', self.state)

	def run_service(self):
		#log_state(self.state, self.transited, self.prev_state) 
		if self.state == 'IDLE':
	    		pass
		if self.state == 'RUN':
